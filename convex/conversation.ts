...
export const get = query({
	...
	handler: async (ctx, args) => {
		...
		if (!conversation.isGroup) {
			...
			const otherMembers = (await Promise.all(allConversationMemberships
				.filter(membership => membership.memberId !== currentUser._id).map(async membership => {
...
export const createGroup = mutation({
	...
	handler: async (ctx, args) => {
		...
		await Promise.all([...args.members, currentUser._id].map(async memberId => {
			await ctx.db.insert("conversationMembers", { memberId, conversationId })
		}))
	}
})
